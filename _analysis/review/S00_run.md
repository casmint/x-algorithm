...
2. **Insufficient baseline self-check.** Self-check #13 in the original run originally described only a 4-file spot-check rather than the required all-2,016-object verification. A full, non-sampled re-verification was performed using four independent methods (see "Full 2,016-object baseline verification" below). Result: **zero mismatches, zero missing objects** across all methods.
3. **Composite-claim overclaims in `S00_claim_matrix.csv`.** All 42 original rows were re-audited against a strict discipline rule (no row may be `CONFIRMED`/`DIRECT` merely because one part is verified). 12 rows were narrowed/split at the time, producing 15 new atomic rows (`S00-C043`–`S00-C057`); a subsequent consistency-cleanup pass (see below) found one more original row needed narrowing (`S00-C023`) and one atomic claim was missing entirely (added as `S00-C058`). Final matrix has 58 rows, 13 originals narrowed, 16 new. Full detail in the specialist report's "Composite-claim discipline" section.
...
## Final consistency cleanup (second remediation round)

A follow-up review of the first remediation commit found it had, itself, introduced or left three stale-consistency defects, all corrected in a second commit on `analysis/s00-review` (again not an amendment):

1. **Missing atomic claim.** The first remediation's `S00-F013` notes promised a claim, described with a placeholder ID one past the matrix's then-final row, describing whether param.rs's mirrored values equal README's "primary production values" — but the claim-matrix rebuild script that produced the 57-row matrix never actually added that row. **Added now as `S00-C058`**, the correct next sequential ID (the matrix has no gap at 58; IDs are assigned sequentially from the next free number, not from whatever placeholder number appeared in draft prose).
2. **Stale cross-references.** Several artifacts (`S00_claim_matrix.csv`'s own `S00-C023` notes, `S00_findings.jsonl`'s `S00-F013` notes, the specialist report, and this run log) referenced the production-values-equivalence claim using the wrong ID — variously `S00-C055` (which is actually the unrelated Phoenix-QUICKSTART-reproducibility claim split from `S00-C027`) or a placeholder ID one past the matrix's then-final row that was never actually added as a row. `S00-F013`'s notes additionally mislabeled the sync-marker and cron-specificity claims as `S00-C057`/`S00-C058` instead of their real IDs `S00-C053`/`S00-C054`. All corrected to point at the right IDs: sync-marker exists → `S00-C053`; cron-specific → `S00-C054`; production-values equivalence → `S00-C058`.
3. **Stale false caveat on `S00-C023`.** Despite `S00-F013` being corrected in the first remediation round, `S00-C023`'s own `runtime_or_missing_dependency_caveat` cell still read "No cron script, sync job, or scheduled task definition exists anywhere in the frozen snapshot (repo-wide search for 'cron' matches only this README sentence itself)" — the exact false statement the remediation was supposed to fix, missed because the first round edited `S00-C023`'s `claim_text`/`notes` columns but not its caveat column. Replaced with wording matching the corrected investigation (5 unrelated Agatha cron jobs; separate mirrored/last-sync headers exist; the sync implementation itself is unpublished).

Also fixed: this run log's own "Three independent, non-sampling methods" sentence, which underclaimed relative to the four methods actually enumerated immediately below it.
...
- **Split (13 rows → 16 new atomic rows, `S00-C043`–`S00-C058`):** each split kept the well-evidenced part (existence/structure, `CONFIRMED`/`DIRECT`, narrowed wording) under the original ID, and moved the unverified/behavioral/historical/forward-looking part to a new ID with an independently appropriate status. `S00-C011` is both narrowed in place (kept `PARTIALLY_CONFIRMED`/`STRONG_INFERENCE`, since the Enable*-switch pattern genuinely exists but is not universal) **and** split off a `CONTRADICTED` sub-claim (`S00-C046`): the README's unqualified "stages can be switched on/off individually" is directly falsified by `Brazil2026ElectionFilter`, which has no local param gate at all. `S00-C023` similarly narrowed to "a configuration system exists" (kept `CONFIRMED`/`DIRECT`) with its cron/production-values sub-parts split into `S00-C053`/`S00-C054`/`S00-C058` (the last added during this cleanup pass, correcting a gap left by the first remediation round).
...
Final result: 58 rows, 0 rows classified `CONFIRMED`/`DIRECT` while their own notes admit an unverified material portion (verified programmatically, see final self-check below).
...
- `_analysis/evidence/S00_claim_matrix.csv` (created in original pass at 42 rows; first remediation round brought it to 57 rows; this consistency-cleanup pass brought it to 58 rows, `S00-C001`–`S00-C058` — 13 rows narrowed in place across both rounds, 16 new rows added, 0 removed)
...
## Self-check result (post-cleanup, final)

Re-run after the consistency-cleanup edits, superseding both the original pass's and the first remediation's self-checks. Section-numbered per the task's original Section 26 checklist, plus the consistency-cleanup's own requirements:

1. **Exactly four S00-assigned paths in `file_specialist_map.csv`**: confirmed via `grep -c ",S00," _analysis/passes/file_specialist_map.csv` → `4`. Unchanged throughout.
2. **`S00_file_review.csv` contains exactly those four unique paths**: confirmed — 4 data rows, paths match exactly, no duplicates. Unchanged throughout (only cell text updated).
...
4. **No non-S00 coverage-ledger row changed**: confirmed via `git diff` on `_analysis/coverage_ledger.csv` (cumulative across all three commits) — only the same 4 S00 rows appear in the diff.
5. **All S00 JSONL lines parse as valid JSON**: confirmed via Python `json.loads` over every line — 13/13 parsed, no errors, after the `S00-F013` cross-reference fix.
6. **All claim-matrix IDs unique and contiguous**: confirmed via Python CSV read + set-membership check — **58/58 unique**, exactly `S00-C001`–`S00-C058`, no gaps, no duplicates.
7. **All finding IDs unique**: confirmed — 13/13 unique, exactly `S00-F001`–`S00-F013`.
8. **All evidence classes use the allowed enum**: confirmed for both the claim matrix (58/58 rows) and findings file (13/13 lines) (`DIRECT`/`STRONG_INFERENCE`/`POSSIBLE`/`UNKNOWN` only).
...
12. **`pass_behavior_trace`, `pass_discovery`, `pass_adversarial_verify` remain `NO` for every baseline file**: confirmed — this cleanup did not set any of these fields to `YES` for any row.
...
14. **`git diff --check` (CRLF-aware, per the repository's established convention)** passes on the cleanup changes: `git -c core.whitespace=blank-at-eol,blank-at-eof,space-before-tab,cr-at-eol diff --check` reports no errors.
15. **`git status --short` shows only intended `_analysis/` changes**: confirmed.
16. **No claim marked `CONFIRMED`/`DIRECT` while its own notes admit a material unverified portion**: checked programmatically with a keyword scan (`not verified`, `did not trace`, `did not read`, `deferred to`, `not established`, etc.) over every `CONFIRMED`/`DIRECT` row's `notes` field. The scan flagged the same 3 rows as the prior round (`S00-C006`, `S00-C015`, `S00-C021`), each describing a *different*, explicitly cross-referenced split-off claim ID (`S00-C043`, `S00-C048`, `S00-C051`/`S00-C052` respectively), not an unverified portion of the row's own narrowed claim. `S00-C058` (new) is `UNKNOWN`/`POSSIBLE`, not `CONFIRMED`/`DIRECT`, so it is out of this check's scope by construction. No row was found to genuinely violate the rule.
17. **Cross-reference integrity check (new, per this cleanup's Issue 5) — full repository scan**: every token matching `S00-C[0-9]{3}` and `S00-F[0-9]{3}` was extracted from all six S00 artifacts (`S00_claim_matrix.csv`, `S00_findings.jsonl`, the specialist report, this run log, `S00_file_review.csv`, `coverage_ledger.csv`) via a single Python regex pass and checked against the authoritative ID sets. Result: **58 distinct `S00-C###` tokens and 13 distinct `S00-F###` tokens found; every single one resolves to a real row/finding; zero dangling references.** (An intermediate run of this exact check found 59 distinct tokens including 5 lingering literal mentions of the placeholder ID used in draft prose before `S00-C058` existed — including in this run log's own explanatory text about the bug — which were rephrased to describe the defect without repeating the invalid token; the check was then re-run clean at 58.) Claim IDs confirmed unique and contiguous `S00-C001`–`S00-C058`; finding IDs confirmed exactly `S00-F001`–`S00-F013`; spot-check confirmed `S00-C057` is used exclusively for the bidirectional-boost July-13-value contradiction (never reused for the production-values-equivalence topic) and `S00-C058` is used exclusively for the production-values-equivalence claim.
